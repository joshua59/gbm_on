 ...
 * Time: 20:10 AM
 */
class pemakaian_mesin_model extends CI_Model{
    ...
    public function data_rekap($key = ''){
        ...
        if ($this->input->post('NF_STATUS')=='3'){
    ...
    public function data_table_rekap($module = '', $limit = 20, $offset = 1) {
        ...
        if (!empty($kata_kunci))
        $filter["(LEVEL4 LIKE '%{$kata_kunci}%' OR NO_TUG LIKE '%{$kata_kunci}%' OR NAMA_JNS_BHN_BKR LIKE '%{$kata_kunci}%')"] = NULL;
        ...
        foreach ($record->result() as $row) {
            ...
            <input type="hidden" id="status" name="status['.$pilih.']" value="'.$row->STATUS_PEMAKAIAN.'">'; 
            ...
            if (($this->session->userdata('level_user') == 2) && ($this->laccess->otoritas('approve'))){
                ...
                if ($row->KODE_STATUS==5){
                    $edit.= anchor(null, '<i class="icon-remove" title="Tolak Data Closing"></i>', array('class' => 'btn transparant', 'id' => 'button-tolak-rkpcls-' . $id, 'onclick' => 'load_form(this.id)', 'data-source' => base_url($module . '/tolak_view_closing/' . $id)));
                }                
            }

            $aksi = $edit;

            $set_status = $row->STATUS_PEMAKAIAN;

            if ($this->input->post('NF_STATUS')=='3'){
                if ($row->IS_TOLAK){
                    $set_status = $row->STATUS_PEMAKAIAN.'<span class="required">*</span>';            
                }
            }                 
            ...
            $rows[$no] = array(
                ...
                'NO PEMAKAIAN' => $row->NO_TUG,
                'TGL PENGAKUAN' => $row->TGL_PENGAKUAN,
                'NAMA JNS BHN BKR' => $row->NAMA_JNS_BHN_BKR,
                'VOL PEMAKAIAN (L)' => number_format($row->VOLUME_PEMAKAIAN,2,',','.'),
    ...
    public function update_notif_tolak($id='') {
        ...
        $this->db->where('CD_BY_MUTASI_PEMAKAIAN', $this->session->userdata('user_name'));
        $this->db->where('IS_TOLAK', '1');
        $this->db->where('ID_PEMAKAIAN', $id);
        $this->db->update('MUTASI_PEMAKAIAN');
    ...
    public function getTableViewDetail($tanggal=null){
    ...
    public function saveDetailPenerimaan($idPenerimaan, $statusPenerimaan,$level_user,$user,$jumlah,$ket=''){
		...
		$query = "call PROSES_PEMAKAIAN('".$idPenerimaan."','".$statusPenerimaan."','".$level_user."','".$user."',".$jumlah.",'".$ket."')";
    ...
    public function saveDetailClossing($sloc,$idPenerimaan,$level_user,$statusPenerimaan,$kode_level,$user_name,$jumlah){
        ...
        $data = $this->db->query("call PROSES_CLOSSING('".$sloc."','".$idPenerimaan."','".$level_user."','".$statusPenerimaan."','".$kode_level."','".$user_name."',".$jumlah.")");
    ...
    public function options_jenis_mesin($default = '--Pilih Jenis Mesin--') {
        ...
        foreach ($list->result() as $row) {
            $option[$row->KODE_MESIN] = $row->NAMA_MESIN;
        }
        $this->db->close();
        return $option;
    }

    public function options_jns_mesin($default = '--Pilih Jenis Mesin--', $id = '') {
        $option = array();
        $this->db->select('MASTER_MESIN.KODE_MESIN, MASTER_MESIN.NAMA_MESIN');
        $this->db->from('MASTER_MESIN');
        $this->db->join('MASTER_LEVEL4', 'MASTER_MESIN.SLOC = MASTER_LEVEL4.SLOC');
        $this->db->where('MASTER_MESIN.IS_AKTIF_MESIN', '1');
        $list = $this->db->get();
