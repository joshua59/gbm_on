...
class permintaan_model extends CI_Model{
    ...
    public function data_detail($key = ''){
        ...
        $sumKirim = ', (select COUNT(NO_NOMINASI) FROM MUTASI_NOMINASI_KIRIM mk WHERE mk.NO_NOMINASI=a.NO_NOMINASI AND mk.SLOC=a.SLOC) AS JML_KIRIM ';
    ...
    public function data_table_rekap($module = '', $limit = 20, $offset = 1) {
        ...
        if (!empty($kata_kunci))
        $filter["(LEVEL4 LIKE '%{$kata_kunci}%' OR NO_NOMINASI LIKE '%{$kata_kunci}%' OR NAMA_PEMASOK LIKE '%{$kata_kunci}%' OR NAMA_JNS_BHN_BKR LIKE '%{$kata_kunci}%')"] = NULL;
        ...
        foreach ($record->result() as $row) {
            ...
            <input type="hidden" id="idPermintaan" name="idPermintaan['.$pilih.']" value="'.$id.'">
            ...
            if (($this->session->userdata('level_user') >= 2) && ($this->laccess->otoritas('add'))){
                if (($row->KODE_STATUS==0) || ($row->KODE_STATUS==4)){ //kirim dan kirim closing
                    ...
                    $edit = anchor(null, '<i class="icon-file-alt" title="Lihat Data"></i>', array('class' => 'btn transparant', 'id' => 'button-view-rkp-' . $id, 'onclick' => 'load_form(this.id)', 'data-source' => base_url($module . '/edit_view/' . $id)));
                    $ceklis = '';
                }                
            } else {
                $edit = anchor(null, '<i class="icon-file-alt" title="Lihat Data"></i>', array('class' => 'btn transparant', 'id' => 'button-view-rkp-' . $id, 'onclick' => 'load_form(this.id)', 'data-source' => base_url($module . '/edit_view/' . $id)));
            ...
            $rows[$no] = array(
                ...
                'NO NOMINASI' => $row->NO_NOMINASI,
                'TGL NOMINASI' => $row->TGL_MTS_NOMINASI,
                'NAMA PEMASOK' => $row->NAMA_PEMASOK,
                'NAMA JNS BHN BKR' => $row->NAMA_JNS_BHN_BKR,
                'VOL NOMINASI (L)' => number_format($row->VOLUME_NOMINASI,2,',','.'),
    ...
    public function update_notif_tolak($id='') {
        ...
        $this->db->where('CD_BY_MTS_NOMINASI', $this->session->userdata('user_name'));
        $this->db->where('IS_TOLAK', '1');
        $this->db->where('ID_PERMINTAAN', $id);
        $this->db->update('MUTASI_NOMINASI');
    ...
    function saveDetailPenerimaan($id, $status, $level_user, $kode_level, $user, $jumlah, $ket=''){
        $query = $this->db->query("call SP_NOMINASI('".$id."','".$status."','".$level_user."','".$kode_level."','".$user."',".$jumlah.",'".$ket."')");
        $this->db->close();
        return $query->result();
    }
    ...
    public function delete_detail($key, $data) {
        $this->db->trans_begin();

	$this->db->where('SLOC', $data);
