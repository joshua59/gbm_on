...
<div class="row-fluid">
    ...
    <div class="box-content">
		...
		<div class="control-group">
			...
			<div class="controls">
                <?php echo form_input('NO_STOCKOPNAME', !empty($default->NO_STOCKOPNAME) ? $default->NO_STOCKOPNAME : '', 'class="span6" placeholder="Ketik Nomor Stok Opname"'); ?>
			</div>
			<br>
			<label for="password" class="control-label">Tanggal BA Stock Opname <span class="required">*</span> : </label>
			<div class="controls">
                <?php echo form_input('TGL_BA_STOCKOPNAME', !empty($default->TGL_BA_STOCKOPNAME) ? $default->TGL_BA_STOCKOPNAME : '', 'class="span2 input-append date form_datetime" placeholder="Pilih Tanggal", id="datepicker"'); ?>
			</div>
			<br>
			<label for="password" class="control-label">Tanggal Pengakuan <span class="required">*</span> : </label>
			<div class="controls">
                <?php echo form_input('TGL_PENGAKUAN', !empty($default->TGL_PENGAKUAN) ? $default->TGL_PENGAKUAN : '', 'class="span2 input-append date form_datetime" placeholder="Pilih Tanggal"'); ?>
			</div>
			<br>
		...
		<div class="control-group">
			...
			<div class="controls">
                <?php echo form_input('VOLUME_STOCKOPNAME', !empty($default->VOLUME_STOCKOPNAME) ? $default->VOLUME_STOCKOPNAME : '', 'class="span3" placeholder="Ketik Volume"'); ?>
			</div>
			
		</div>
		<div class="md-card-content">
			<label for="password" class="control-label">Upload File (Maks 4 MB) : </label>
			<div class="controls">
...
<script type="text/javascript">
    ...
    function cekTanggalBa(){
		...
		$('input[name=TGL_PENGAKUAN]').datepicker('setStartDate', '2017-01-01');
    ...
    function setDefaulthTglBa(){
        ...
        $('input[name=TGL_BA_STOCKOPNAME]').datepicker('setStartDate', '2017-01-01');
    ...
    function checkDefaulthTglBa(){
        ...
        if (vDateStart < '20170101') {
            var message = '<div class="box-title" style="color:#ac193d;"><i class="icon-remove-sign"></i>  Tanggal Berita Acara tidak boleh kurang dari tahun 2017</div>';
            bootbox.alert(message, function() {});
            $('input[name=TGL_BA_STOCKOPNAME').datepicker('update', '2017-01-01');
            
        }        		
	}

    function cekMinTglPengakuan(){
        var tgl_pengakuan = $("input[name=TGL_PENGAKUAN]").val();
        tgl_pengakuan = tgl_pengakuan.replace(/-/g, '');

        if (tgl_pengakuan < '20170101') {
            ...
            bootbox.alert(message, function() {});
            $('input[name=TGL_PENGAKUAN').datepicker('update', '2017-01-01');
    ...
    $("input[name=TGL_PENGAKUAN]").change(cekMinTglPengakuan);
    ...
    $('input[name=VOLUME_STOCKOPNAME]').inputmask("numeric", {radixPoint: ",",groupSeparator: ".",digits: 2,autoGroup: true,prefix: '',rightAlign: false,allowMinus: false, oncleared: function () { self.Value(''); }
	});
