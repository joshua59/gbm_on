                    ...
                    <div class="well-content clearfix">
                        ...
                        <div class="form_row">
                            ...
                            <div class="pull-left span3">
                                ...
                                <label for="password" class="control-label" style="margin-left:95px">Tahun : </label>
                                <div class="controls">
...
<script type="text/javascript">
          ...
          $.post("<?php echo base_url()?>data_transaksi/pemakaian/getDataDetail/", data, function (data) {
              ...
              for (i = 0; i < data_detail.length; i++) {
                  ...
                  vEditView = '<a href="javascript:void(0);" class="btn transparant" id="button-edit-'+data_detail[i].ID_PEMAKAIAN+'" onclick="load_form(this.id)" data-source="'+vlink_url+'"> <i class="icon-file-alt" title="Lihat Data"></i></a>'; 
                    ...
                    vlink_url = "<?php echo base_url()?>data_transaksi/pemakaian/tolak_view/"+data_detail[i].ID_PEMAKAIAN;
                    vTolakView = '<a href="javascript:void(0);" class="btn transparant" id="button-tolak-'+data_detail[i].ID_PEMAKAIAN+'" onclick="load_form(this.id)" data-source="'+vlink_url+'"> <i class="icon-remove" title="Tolak Data"></i></a>';

                    vlink_url = "<?php echo base_url()?>data_transaksi/pemakaian/tolak_view_closing/"+data_detail[i].ID_PEMAKAIAN;
                    vTolakViewClosing = '<a href="javascript:void(0);" class="btn transparant" id="button-tolakcls-'+data_detail[i].ID_PEMAKAIAN+'" onclick="load_form(this.id)" data-source="'+vlink_url+'"> <i class="icon-remove" title="Tolak Data Closing"></i></a>';                    
                  ...
                  if (vLevelUser>=2){
                      if (vLevelUser==2){
                          if (vIsAdd){
                              if((data_detail[i].KODE_STATUS == "1") || (data_detail[i].KODE_STATUS == "2") || (data_detail[i].KODE_STATUS == "3") || (data_detail[i].KODE_STATUS == "6") || (data_detail[i].KODE_STATUS == "7")){ 
                                  cekbox = '';  
                              } else {
    ...
    function saveDetailTolakClossing(obj) {
      if (cekChekBoxPilih('tolak')){return;}
      var url = "<?php echo base_url() ?>data_transaksi/pemakaian/saveKirimanClossing/tolak";
      bootbox.confirm('Yakin data ini akan ditolak ?', "Tidak", "Ya", function(e) {
        if(e){
          bootbox.modal('<div class="loading-progress"></div>');
          $.ajax({
