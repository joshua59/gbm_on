...
class tutup_mutasi_persediaan extends MX_Controller {
    ...
    public function add_buka($id = '') {
        ...
        $data['reg_options'] = $this->tbl_get->options_reg();
        $data['blth_options'] = $this->tbl_get->options_blth($data['default']->BLTH);
        $data['options_tahun'] = $this->tbl_get->options_tahun(); 
        $data['options_bulan'] = $this->tbl_get->options_bulan();    
    ...
    public function proses() {
        ...
        if ($this->form_validation->run($this)) {
            ...
            if ($id == '') {
                    $data['STATUS'] = "1";     
                    if ($this->tbl_get->cek_periode($data['BLTH'])) {
                        $message = array(false, 'Proses gagal', 'BLTH '.$data['BLTH'].' sudah ada pada sistem ', '');
                    } else if ($this->tbl_get->save_as_new($data)) {
    ...
    public function proses_buka() {
        ...
        if ($this->form_validation->run($this)) {
            ...
            if ($this->session->userdata('roles_id') == 20 || $this->session->userdata('roles_id') == 34) {
                $data['BLTH']= $this->input->post('BLTH');
                $data['ID_BUKA_MUTASI'] = '00000'.$data['PLANT'].$data['BLTH'];
            } else if($this->session->userdata('roles_id') == 001 || $this->session->userdata('roles_id') == 26) {
            ...
            if ($id == '') {      
                $cek_blth = $this->tbl_get->cek_buka_mutasi($data['BLTH'],$data['PLANT']);
                if ($cek_blth) {
                    $message = array(false, 'Proses gagal', 'Data Buka Mutasi '.$cek_blth.' Periode '.$data['BLTH'].' Telah Tersedia, Silahkan Edit Data Untuk Open Periode Kembali.', '');
                } else {
                ...
                $cek_blth = $this->tbl_get->cek_buka_mutasi($data['BLTH'],$data['PLANT'],$id);
                if ($cek_blth) {
                    $message = array(false, 'Proses gagal', 'Data Buka Mutasi '.$cek_blth.' Periode '.$data['BLTH'].' Telah Tersedia, Silahkan Edit Data Untuk Open Periode Kembali.', '');                    
                } else {
            ...
            $message = array(true, 'Proses gagal', validation_errors(), '');
        }
        echo json_encode($message, true);
    }
