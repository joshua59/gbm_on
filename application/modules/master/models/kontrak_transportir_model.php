...
class kontrak_transportir_model extends CI_Model {
	...
	public function data($key = '') {
        ...
        if ($kata_kunci){
            $this->db->where("(b.NAMA_TRANSPORTIR LIKE '%$kata_kunci%' OR a.KD_KONTRAK_TRANS LIKE '%$kata_kunci%')");
        }              
    ...
    public function delete_detail_by_kd($key) {    
        $this->db->trans_begin();
    ...
    public function delete_detail($key) {
        $q="SELECT KD_KONTRAK_TRANS 
            FROM DATA_KONTRAK_TRANSPORTIR WHERE ID_KONTRAK_TRANS='$key' ";
        
        $query = $this->db->query($q)->result_array();
        $kd_kontrak = $query[0]['KD_KONTRAK_TRANS']; 
        // print_r($query[0]['KD_KONTRAK_TRANS']); die;
    
        $this->db->trans_begin();
        $this->db->delete('DET_KONTRAK_TRANS', array('KD_KONTRAK_TRANS' => $kd_kontrak)); 
    ...
    public function get_detail_kirim($key) {
		$q="SELECT a.HARGA_KONTRAK_TRANS, a.SLOC, a.ID_DEPO, a.ID_DEPO_TRANSIT, a.JARAK_DET_KONTRAK_TRANS, a.TYPE_KONTRAK_TRANS, 
	...
	public function options_denda($default = '--Pilih Mekanisme--') {
	    $this->db->from('M_DENDA');
	    $this->db->order_by('ID_DENDA');
	    ...
	    foreach ($list->result() as $row) {
	        $option[$row->ID_DENDA] = $row->NAMA_DENDA;
	    }
	    $this->db->close();
	    return $option;
	}	
