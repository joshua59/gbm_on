...
class tangki extends MX_Controller {
    ...
    public function proses() {
        ...
        if ($x>0){
            ...
            for ($i=1; $i<=$x; $i++) {
                ...
                $this->form_validation->set_rules('DEADSTOCK_TANGKI'.$i, 'Dead Stock (L) ke '.$i, 'required');
                $this->form_validation->set_rules('STOCKEFEKTIF_TANGKI'.$i, 'Kapasitas Mampu (L) ke '.$i, 'required');

                //cek validasi tera
                if (($this->input->post('TGL_AWAL_TERA'.$i)) || ($this->input->post('TGL_AKHIR_TERA'.$i)) || ($_FILES['PATH_DET_TERA'.$i]['name'])){
                    $this->form_validation->set_rules('TGL_AWAL_TERA'.$i, 'Tgl Awal Tera ke '.$i, 'required');
                    $this->form_validation->set_rules('TGL_AKHIR_TERA'.$i, 'Tgl Akhir Tera ke '.$i, 'required');
                    // $this->form_validation->set_rules('PATH_DET_TERA'.$i, 'Upload File ke '.$i, 'required');
                    ...
                    if ($id==''){
                        ...
                        if ($this->input->post('PATH_FILE_EDIT'.$i)==''){
                            if (empty($_FILES['PATH_DET_TERA'.$i]['name'])){
                                $this->form_validation->set_rules('PATH_DET_TERA'.$i, 'Upload File ke '.$i, 'required');
                            }
                        }    
        ...
        if ($this->form_validation->run($this)) {
            ...
            for ($i=1; $i<=$x; $i++){
                ...
                // $old_name = $this->input->post('PATH_FILE_EDIT'.$i);
                ...
                //     if ($old_name!=''){
                //         $target='assets/upload/tangki/'.$old_name;
                ...
                //         // $_prod = $this->laccess->post_file_prod('TANGKI',$nama_file);
                ...
                if ($this->input->post('TGL_AWAL_TERA'.$i)){
                    $tgl_awal = $this->input->post('TGL_AWAL_TERA'.$i);
                } else {
                    $tgl_awal = null;
                }

                if ($this->input->post('TGL_AKHIR_TERA'.$i)){
                    $tgl_akhir = $this->input->post('TGL_AKHIR_TERA'.$i);
                } else {
                    $tgl_akhir = null;
                }
                ...
                if ($id == '') {
                    $data_detail[$i] = array(
                        ...
                        'TGL_AWAL_TERA' => $tgl_awal, //$this->input->post('TGL_AWAL_TERA'.$i),
                        'TGL_AKHIR_TERA' => $tgl_akhir, //$this->input->post('TGL_AKHIR_TERA'.$i),
                    ...
                    if ($this->input->post('CREATE_BY'.$i)){
                        $cd_by = $this->input->post('CREATE_BY'.$i);
                        $cd_date = $this->input->post('CREATE_DATE'.$i); 
                        $ud_by = $this->session->userdata('user_name');
                        $ud_date = date('Y-m-d'); 
                    } else {
                        $cd_by = $this->session->userdata('user_name');
                        $cd_date = date('Y-m-d'); 
                        $ud_by = null;
                        $ud_date = null;     
                    }
            ...
            $data['IS_AKTIF_TANGKI'] = $ceklis;
           
            if ($id == '') {
                $data['CD_TANGKI'] = date("Y/m/d");
                $data['CD_BY_TANGKI'] = $this->session->userdata('user_name');
                ...
                //p_tangki = 1 master nya
                //p_tangki = 2 detail nya

                $p_sloc = $data['SLOC'];
                $p_idtangki = $id;
                $p_iddettangki = '';
                $p_nmtangki = '';
                $p_idbbm = $data['ID_JNS_BHN_BKR'];
                $p_teraby = '';
                $p_path = '';
                $p_tglawal = '';
                $p_tglakhir = '';
                $p_volume = $data['VOLUME_TANGKI'];
                $p_deadstock = $data['DEADSTOCK_TANGKI'];
                $p_stockefektif = $data['STOCKEFEKTIF_TANGKI'];
                $p_user = $this->session->userdata('user_name');
                $p_aktif = '1';
                $p_tangki = '1';
            
                // for ($i=1; $i<=$x; $i++){
                //     echo $data_detail[$i]['NAMA_TANGKI'];
                //     // echo '\n';
                // }
                ...
                if ($res ==''){                    
                    ...
                    //simpan master 
                    $simpan = $this->tangki_model->call_edit_tangki($p_sloc, $p_idtangki, $p_iddettangki, $p_nmtangki, $p_idbbm, $p_teraby, $p_path, $p_tglawal, $p_tglakhir, $p_volume, $p_deadstock, $p_stockefektif, $p_user, $p_aktif, $p_tangki);
                    ...
                    if ($simpan[0]->RCDB == "RC00") {
                        ...
                        $message = array(true, 'Proses Berhasil ', $simpan[0]->PESANDB.' 
                            <br>
                            Upload File :
                            <br> 
                            - Berhasil ['.$berhasil. '] 
    ...
    public function delete($id) {
        ...
        $this->tangki_model->delete_detail($id);
    ...
    public function cek_jns_bbm() {
        $val = array(
            'ID_JNS_BHN_BKR' => $this->input->post('ID_JNS_BHN_BKR'),
            'SLOC' => $this->input->post('SLOC'),
        );

        $message = $this->tangki_model->cek_jns_bbm($val);
...
/* Location: ./application/modules/tangki/controllers/pemasok.php */
