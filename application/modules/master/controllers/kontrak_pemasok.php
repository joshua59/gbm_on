...
class kontrak_pemasok extends MX_Controller {
    ...
    public function index() {
        ...
        $this->asset->set_plugin(array('crud','format_number','select2','maxlength'));
    ...
    public function add($id = '') {
        ...
        $data['jns_bhn_bkr_options'] = $this->tbl_get->options_jns_bhn_bkr();
        $data['jns_isidentil_options'] = $this->tbl_get->options_isidentil();
        $data['page_title'] = '<i class="icon-laptop"></i> ' . $page_title;
        $data['form_action'] = base_url($this->_module . '/proses');
		$data["url_getfile"] = $this->_urlgetfile;
        $data['lv4_options'] = $this->tbl_get->options_lv4('--Pilih Pembangkit--', 'all', 0);
    ...
    public function proses() {
        ...
        $this->form_validation->set_rules('NOPJBBM_KONTRAK_PEMASOK', 'No PJBBBM', 'trim|required|max_length[200]');
        $this->form_validation->set_rules('JUDUL_KONTRAK_PEMASOK', 'Judul Kontrak', 'required');
        $this->form_validation->set_rules('PERIODE_AWAL_KONTRAK_PEMASOK', 'Periode Awal', 'required');
        $this->form_validation->set_rules('PERIODE_AKHIR_KONTRAK_PEMASOK', 'Periode Akhir', 'required');
        if ($this->input->post('JENIS_KONTRAK_PEMASOK') == '1') {
            $this->form_validation->set_rules('SLOC', 'Pembangkit', 'required');
        }
        $this->form_validation->set_rules('JENIS_KONTRAK_PEMASOK', 'Jenis Kontrak', 'required');
        ...
        if ($this->form_validation->run($this)) {
            ...
            $data['SKEMA_ISIDENTIL'] = $this->input->post('SKEMA_ISIDENTIL');
            ...
            if ($data['VOLUME_KONTRAK_PEMASOK']==''){$data['VOLUME_KONTRAK_PEMASOK']=null;}
            ...
            if ($data['RUPIAH_KONTRAK_PEMASOK']==''){$data['RUPIAH_KONTRAK_PEMASOK']=null;}
            ...
            if ($data['NOMINAL_JAMINAN_KONTRAK']==''){$data['NOMINAL_JAMINAN_KONTRAK']=null;}

            $data['TGL_BERAKHIR_JAMINAN_KONTRAK'] = $this->input->post('TGL_BERAKHIR_JAMINAN_KONTRAK');    
            if ($data['TGL_BERAKHIR_JAMINAN_KONTRAK']==''){$data['TGL_BERAKHIR_JAMINAN_KONTRAK']=null;}
            ...
            if ($id == '') {
                ...
                if (!$this->upload->do_upload('ID_DOC_PEMASOK')){
                    ...
                    if ($cek_file){
                        ...
                        $message = array(false, 'Proses Simpan Gagal', '');
                    }
                }
            } else {
                ...
                if (!empty($_FILES['ID_DOC_PEMASOK']['name'])){
                    ...
                    if (!$this->upload->do_upload('ID_DOC_PEMASOK')){
                        ...
                        $set_file = $this->upload->data();
                        if ($set_file){
                            $nama_file= $set_file['file_name'];
    ...
    public function proses_adendum() {
        $this->form_validation->set_rules('ID_KONTRAK_PEMASOK', 'Pemasok', 'trim|required|max_length[20]');
        $this->form_validation->set_rules('NO_ADENDUM_PEMASOK', 'No Adendum', 'trim|required|max_length[200]');
        $this->form_validation->set_rules('KET_ADENDUM_PEMASOK', 'Keterangan Adendum', 'trim|required|max_length[200]');
        $this->form_validation->set_rules('TGL_ADENDUM_PEMASOK', 'Tgl Kontrak', 'required');
        $this->form_validation->set_rules('JUDUL_ADENDUM_PEMASOK', 'Judul Kontrak', 'required');
        $this->form_validation->set_rules('PERIODE_AWAL_ADENDUM_PEMASOK', 'Periode Awal', 'required');
        $this->form_validation->set_rules('PERIODE_AKHIR_ADENMDUM_PEMASOK', 'Periode Akhir', 'required');
        $this->form_validation->set_rules('JENIS_AKHIR_ADENDUM_PEMASOK', 'Jenis Kontrak', 'required');
        if ($this->input->post('JENIS_AKHIR_ADENDUM_PEMASOK') == '1') {
            $this->form_validation->set_rules('SLOC', 'Pembangkit', 'required');
        }
        ...
        if ($this->form_validation->run($this)) {
            ...
            if ($data['VOL_AKHIR_ADENDUM_PEMASOK']==''){$data['VOL_AKHIR_ADENDUM_PEMASOK']=null;}
            ...
            if ($data['RP_ADENDUM_PEMASOK']==''){$data['RP_ADENDUM_PEMASOK']=null;}
            ...
            if ($data['NOMINAL_ADENDUM_PEMASOK']==''){$data['NOMINAL_ADENDUM_PEMASOK']=null;}

            $data['TGL_AKHIR_ADENDUM_PEMASOK'] = $this->input->post('TGL_AKHIR_ADENDUM_PEMASOK');
            if ($data['TGL_AKHIR_ADENDUM_PEMASOK']==''){$data['TGL_AKHIR_ADENDUM_PEMASOK']=null;}
