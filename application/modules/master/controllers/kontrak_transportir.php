...
class kontrak_transportir extends MX_Controller {
    ...
    public function proses() {
        ...
        $this->form_validation->set_rules('KD_KONTRAK_TRANS', 'Nomor Kontrak Transportir', 'trim|required|max_length[200]');
        ...
        $this->form_validation->set_rules('ID_DENDA', 'Mekanisme Denda', 'trim|required|');
        $this->form_validation->set_rules('KET_DENDA', 'Keterangan Mekanisme Denda', 'trim|required|');
        // if ($this->input->post('ID_DENDA')=='9999'){
        //     $this->form_validation->set_rules('KET_DENDA', 'Keterangan Mekanisme Denda', 'trim|required|');
        // }
        $this->form_validation->set_rules('LOSSES', 'Toleransi Losses', 'trim|required|');
        ...
        if ($x>0){
            ...
            for ($i=1; $i<=$x; $i++) {
                ...
                if (($this->input->post('depotransit_ke'.$i)=='')&&($this->input->post('pembangkit_ke'.$i)=='')){
                    $this->form_validation->set_rules('pembangkit_ke'.$i, 'Depo Transit Penerima ke '.$i.' atau KIT Penerima ke '.$i, 'required');                
                }                
        ...
        if ($this->form_validation->run($this)) {
            ...
            $data['LOSSES'] = str_replace(".","",$this->input->post('LOSSES'));
            $data['LOSSES'] = str_replace(",",".",$data['LOSSES']);
            $data['ID_DENDA'] = $this->input->post('ID_DENDA');
            $data['KET_DENDA'] = $this->input->post('KET_DENDA');
            ...
            for ($i=1; $i<=$x; $i++)
            {
                ...
                if ($this->input->post('depotransit_ke'.$i)){
                    $depotransit_ke = $this->input->post('depotransit_ke'.$i);
                } else {
                    $depotransit_ke = '';
                }

                if ($this->input->post('pembangkit_ke'.$i)){
                ...
                $data_detail[$i] = array(
                    ...
                    'ID_DEPO' => $depo_ke,
                    'ID_DEPO_TRANSIT' => $depotransit_ke,
                    'SLOC' => $pembangkit_ke,
            ...
            if ($id == '') {
                ...
                if (!$this->upload->do_upload('FILE_UPLOAD')){
                    ...
                    if ($res){
                        ...
                        $message = array(false, 'Proses Simpan Gagal', $simpan_data_detail, '');
                    }
                }
            } else {
                ...
                if (empty($_FILES['FILE_UPLOAD']['name'])){
                    if ($this->kontrak_transportir_model->save($data, $id)) {
                        ...
                        $simpan_data_detail = $this->kontrak_transportir_model->delete_detail_by_kd($data['KD_KONTRAK_TRANS']);
                        if ($x>0){
                            $simpan_data_detail = $this->kontrak_transportir_model->save_detail($data_detail);    
                        }                        
    ...
    public function add_adendum($id = '') {
        ...
        $data['option_depo'] = $this->kontrak_transportir_model->getDepo();
        $data['option_jalur'] = $this->kontrak_transportir_model->getJalur();
        $data['option_denda'] = $this->kontrak_transportir_model->options_denda();
        $data['page_title'] = '<i class="icon-laptop"></i> ' . $page_title;
    ...
    public function proses_adendum() {
        $this->form_validation->set_rules('ID_TRANSPORTIR', 'Transportir', 'trim|required');
        $this->form_validation->set_rules('NO_KONTRAK', 'Nomor Adendum Kontrak Transportir', 'trim|required|max_length[200]');
        $this->form_validation->set_rules('KETERANGAN', 'Keterangan adendum', 'trim|required|max_length[200]');
        $this->form_validation->set_rules('TGL_KONTRAK_TRANS', 'Tgl Awal Kontrak', 'trim|required');
        $this->form_validation->set_rules('TGL_KONTRAK_TRANS_AKHIR', 'Tgl Akhir Kontrak', 'trim|required');        
        ...
        if ($this->form_validation->run($this)) {
            ...
            $data['NILAI_KONTRAK_TRANS'] = str_replace(",",".",$data['NILAI_KONTRAK_TRANS']);
            $data['KET_KONTRAK_TRANS'] = $this->input->post('KETERANGAN');
            // $data['KD_KONTRAK_TRANS'] = $this->laccess->ai($this->input->post('NO_KONTRAK'));
            $data['KD_KONTRAK_TRANS'] = str_replace("'","",$this->input->post('NO_KONTRAK'));
            $data['KD_KONTRAK_TRANS'] = str_replace('"','',$data['KD_KONTRAK_TRANS']);
