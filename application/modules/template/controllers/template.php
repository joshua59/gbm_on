...
class template extends MX_Controller
{
    ...
    public function get_ktbi()
    {
        ...
        $test  = $html->find('span[id=ctl00_PlaceHolderMain_g_6c89d4ad_107f_437d_bd54_8fda17b556bf_ctl00_MataUang]');

        foreach ($html->find('span[id=ctl00_PlaceHolderMain_g_6c89d4ad_107f_437d_bd54_8fda17b556bf_ctl00_MataUang]') as $last_) {
            $last_update = $last_->innertext;
        }
    ...
    public function set_max_pemakaian(){
        $blth = date("Ym");
        $get = $this->template_model->set_max_pemakaian($blth);        
        print_r('#'.date("Y-m-d H:i:s").'  #Rest set max pemakaian = '.' \r\n');      
        print_r($get[0]->RCDB.' - '.$get[0]->PESANDB);  
    }       
    ...
    public function get_gm_email()
    {
        ...
        foreach ($record as $row) {
            ...
            // echo $isi_email_html.'<br><hr><br>'; die();
    ...
    public function insert_pemakaian() {
        $exec = $this->template_model->insert_pemakaian();

        if($exec->MESSAGE == 'SUCCESS') {
            $data_job['TIPE_JOB'] = 'INSERT_PEMAKAIAN';
            $data_job['PESAN'] = 'Berhasil input max pemakaian';
            $data_job['STATUS'] = 1;
            $data_job['CD_DATE'] = date('Y-m-d H:i:s');
            $this->template_model->save_as_new_monjob($data_job);   
        } else {
            $data_job['TIPE_JOB'] = 'INSERT_PEMAKAIAN';
            $data_job['PESAN'] = 'Gagal input max pemakaian';
            $data_job['STATUS'] = 2;
            $data_job['CD_DATE'] = date('Y-m-d H:i:s');
            $this->template_model->save_as_new_monjob($data_job);   
        }
    ...
    function kirim_email_persediaan()
    {
        ...
        $x_berhasil = 0;
        $x_gagal = 0;
        $record = $this->template_model->get_level();
        // print_r($record); die();
        foreach ($record as $row) {
            $data = $this->template_model->kirim_email_persediaan($row->code, $row->lvl);
            $d['array'] = $data;
            $d['subject'] = 'GBMO - Hari Operasi Pembangkit (HOP) lebih dari 15 hari pada Aplikasi GBM Online';
            $d['pltd'] = $row->NAMA;
            $d['nama_user'] = $row->USERNAME;
            // $from = "baktidwijaya@gmail.com";
            // $from = "gbmo.pln@pln.co.id";
            $to = $row->EMAIL_USER;
            $subject = 'GBMO - Hari Operasi Pembangkit (HOP) lebih dari 15 hari pada Aplikasi GBM Online';
            $message = $this->load->view('template/kirim_email_persediaan', $d, TRUE);
            $this->email->initialize($config);
            
            $this->email->from('noreplybbmonline@gmail.com', 'Servicedesk GBMO');
            $this->email->to($to);
            $this->email->subject($subject);
            $this->email->message($message);
