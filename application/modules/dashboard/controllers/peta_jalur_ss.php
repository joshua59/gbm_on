...
/**
 * @module Dashboard Peta Jalur Pasokan duplicate
 */
class peta_jalur_ss extends MX_Controller {

    private $_title = 'DASHBOARD PETA';
    private $_limit = 10;
    private $_module = 'dashboard/peta_jalur_ss';
    ...
    public function __construct() {
        ...
        /* Load Global Model */
        $this->load->model('peta_jalur_model', 'tbl_get');
    }
    ...
    public function index() {
        ...
        $data = $this->get_level_non_user();

        $data['hop'] = array('' => '-- Pilih HOP --','1' => 'Merah', '2' => 'Kuning', '3' => 'Hijau');
    ...
    public function get_peta() {
        $rest = $this->tbl_get->call_peta();
        // print_r($rest);die();
        echo json_encode($rest);
    }

    public function get_peta_non_depo() {
        $rest = $this->tbl_get->call_peta_non_depo();
        // print_r($rest);die();
        echo json_encode($rest);
    }

    public function get_peta_ss() {
        $rest = $this->tbl_get->call_peta_ss();
        echo json_encode($rest);
    }
    ...
    public function get_jalur_lama() {
        ...
        $rest = $this->tbl_get->get_jalur($jenis, $id);
        echo json_encode($rest);
    }

    public function get_jalur() {        
        $rest = $this->tbl_get->call_jalur();
        echo json_encode($rest);
    }

    public function get_hop() {        
        $rest = $this->tbl_get->get_hop();
        echo json_encode($rest);
    }

    public function load($page = 1){
        $rest = $this->tbl_get->get_peta();
        echo json_encode($rest);
    }

    public function get_level_non_user(){
        $data['reg_options'] = $this->tbl_get->options_reg_ss();   
    ...
    public function get_options_lv1($key=null) {
        $message = $this->tbl_get->options_lv1_ss('--Pilih Level 1--', $key, 0);
        echo json_encode($message);
    }

    public function get_options_lv2($key=null) {
        $message = $this->tbl_get->options_lv2_ss('--Pilih Level 2--', $key, 0);
        echo json_encode($message);
    }
    ...
    public function get_options_lv4($key=null) {
        $message = $this->tbl_get->options_lv4_ss('--Pilih Pembangkit--', $key, 0);
        echo json_encode($message);
    }
