...
class persediaan_bbm extends MX_Controller {
    ...
    public function ajax_list(){
        ...
        foreach ($list as $table) {
            ...
            $row[] = number_format($table->TERIMA_PENGEMBALIAN,2,',','.');
            ...
            $row[] = number_format($table->STOCKEFEKTIF_TANGKI,2,',','.');
            $row[] = number_format($table->VOLUME_TANGKI,2,',','.');
            ...
            $row[] = number_format($table->ULLAGE,2,',','.');
            if ($table->SHO!=null){
                $row[] = number_format($table->SHO,2,',','.');
            } else {
                $row[] = '<font size="5">~</font>';
            }
            $row[] = $table->ID_MUTASI_PERSEDIAAN;
    ...
    public function export_pdf(){
        ...
        $this->lpdf->html($html);
        $this->lpdf->nama_file('Laporan_Persediaan_BBM.pdf');
