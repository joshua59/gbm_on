...
class pemakaian extends MX_Controller
{
    ...
    public function export_pdf()
    {
        ...
        $this->lpdf->nama_file('Laporan_Pemakaian_BBM.pdf');
        $this->lpdf->cetak('A4');
    }
    ...
    public function export_pdf_detail()
    {
        ...
        $this->load->library('lpdf');
        $this->lpdf->html($html_source);
        $this->lpdf->nama_file('Laporan_Detail_Pemakaian_BBM.pdf');
    ...
    public function generateXls() {
            ...
            $data              = array(
            // 'LVL0'             => $this->input->post('xlvl'),
            'ID_REGIONAL'      => $this->input->post('clvl'), // 01
            'idRegional'       => $this->input->post('clvl0'), // 01
        ...
        $listInfo = $this->tbl_pemakaian_get->exportList($data);
        $objPHPExcel = new PHPExcel();
        $objPHPExcel->setActiveSheetIndex(0);
        // set Header
        $objPHPExcel->getActiveSheet()->SetCellValue('B2', 'Unit');
        $objPHPExcel->getActiveSheet()->SetCellValue('C2', 'Jenis Bahan Bakar');
        $objPHPExcel->getActiveSheet()->SetCellValue('D2', 'Jumlah Pakai');
        $objPHPExcel->getActiveSheet()->SetCellValue('E2', 'Tgl Awal Pakai'); 
        $objPHPExcel->getActiveSheet()->SetCellValue('F2', 'Tgl Akhir Pakai');  
        $objPHPExcel->getActiveSheet()->SetCellValue('G2', 'Total Volume Pemakaian (L)');  
        ...
        foreach ($listInfo as $list) {
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $list['JMLH_PAKAI']);
            $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $list['TGL_AWAL_PAKAI']);
            $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $list['TGL_AKHIR_PAKAI']);
            $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $list['VOLUME_PEMAKAIAN']);
    
            $rowCount++;
        }
        
        $filename = "Laporan_Pemakaian". date("Y-m-d-H-i-s").".csv";
