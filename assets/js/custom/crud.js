...
function load_table(id, page, callback) {
    ...
    $.post(link + '/' + page, data, function(res) {
        ...
        //start untuk custom header table mops
        if($("#ID_MOPS").length != 0) {
            document.getElementById("ID_MOPS").deleteTHead();
            var myTable = jQuery("#ID_MOPS");
            var thead = myTable.find("thead");
            
            var vthead = "<thead><tr><th rowspan='2'>NO</th><th rowspan='2'>TANGGAL</th><th colspan='2'>HSD</th><th colspan='2'>MFO HSFO</th><th colspan='2'>MFO LSFO</th><th rowspan='2'>HITUNG</th><th rowspan='2'>AKSI</th></tr><tr><th>LOW</th><th>MID</th><th>LOW</th><th>MID</th><th>LOW</th><th>MID</th></tr></thead>";
            if (thead.length===0){ 
            thead = jQuery(vthead).appendTo(myTable);    
            }          
        }
        //end untuk custom header table mops

    }, type_data).done(function() {
        // alert( "success" );
        if (id=='#content_table'){
            //update notif
        ...
        if (id=='#content_table2'){
            // if (typeof update_notif_tolak !== 'undefined' && $.isFunction(update_notif_tolak)) {                
            //     update_notif_tolak();                
            // } else {
                if (typeof cek_notif !== 'undefined' && $.isFunction(cek_notif)) {
                    cek_notif();
    ...
    //     $(id).removeClass('loading-progress').html(content);
    //     // console.log(xhr);
    //     alert('Proses gagal,  '+status+' - '+error);
    });
}
...
//            table += '  <tr id="content_row_'+idx_row+'" '; //old
            ...
            if (obj.drildown === true) {
                table += '<tr class="nodrop nodrag hide" id="drildown_row_' + obj.id + '_' + idx_row + '" status="close"><td style="background-color:#ccc!important;" colspan="' + jumlah_kolom + '"></td></tr>';
            }
        });
    } else {
...
function simpan_data_notif(button, form, disable, content) {
    if (typeof $('#' + button).attr('disabled') === 'undefined') {
...
function aktivasi_user(id, ket) {
    ...
    bootbox.confirm('Anda yakin akan '+ ket +' user ini?', "Tidak", "Ya", function(e) {
        if (e) {
            disabled_html([temp_id], true);
            bootbox.modal('<div class="loading-progress"></div>');
            $.post(link, function(res) {
...
function confirm_dialog_ajax(id) {
	...
	var reasonHtml = ' <div class="control-group">';
		reasonHtml += '<label for="password" class="control-label">Alasan</label>';
		reasonHtml += '<div class="controls">';
        reasonHtml += '<input class="span5" id="txttest" type="text" value="" name="txttest"></input>';
        reasonHtml += '</div>';
        reasonHtml += '</div>';
    ...
    bootbox.confirm(msg, "Tidak", "Ya", function(e) {
        if (e) {
			...
			//Jika Button Reject
			if(id == 'button-reject'){
				bootbox.confirm(reasonHtml, "Cancel", "Submit", function(e) {
					if (e){
						// alert(document.getElementById('txttest').value);
						var reason =  document.getElementById('txttest').value;
						bootbox.modal('<div class="loading-progress"></div>');
						$.post(link, { param: reason},function(res) {
							var message = '';
							var icon = 'icon-remove-sign';
							var color = '#ac193d;';
							var content_id = res[3];
...
function zoom_tulisan(id) 
{
    var temp_id = '#' + id;
    var link = $(temp_id).attr('data-source');
    var msg = $(temp_id).attr('data-message');
	
    var tampilIsi = ' <div class="control-group">';
        tampilIsi += '<label for="password" class="control-label">Isi</label>';
        tampilIsi += '<div class="controls">';
        // tampilIsi += '<input class="span5" id="txttest" type="text" value="" name="txttest"></input>';
        tampilIsi += '<span class="span5" id="txttest" type="text" value="" name="txttest">'+msg;
        tampilIsi += '</span>';
        tampilIsi += '</div>';
        tampilIsi += '</div>';
    
    bootbox.alert(tampilIsi);
...
//hide by tri
//
//function close_form_insert(form) {
//    var source = $(form).attr('data-source');
//    window.location = source + 'kms_main/ate/';
//}
...
//Function Load Tree Menu Non Checkbox
function loadTree(id, idtable){
				var arrChild=[];
				var arrParent=[];
				var nameParent;
				var link = $(id).attr('data-source');
                $(function() {
                    ...
                    $(id).aciTree({
						...
						itemHook: function(parent, item, itemData, level) {
							if (!itemData.inode) {
								item.attr('title', 'Jumlah: ' + itemData.value + ' Data').tooltip({
									track: true,
									content: function() {
										// show HTML content
				 ...
				 $(id).on('acitree', function(event, api, item, eventName, options) {
                         switch (eventName) {
							case 'init':
								...
								break;
							case 'selected':
								$("#map_id").val(api.getId(item));
								load_table(idtable, 1);
								break;
						}
                    });
}

//Function Load Tree Menu With Checkbox
function loadTreeCheckbox(id){
	var arrChild=[];
	var arrParent=[];
	var nameParent;
	var link = $(id).attr('data-source');
