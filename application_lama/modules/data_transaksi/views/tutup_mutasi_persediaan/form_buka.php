    ...
    <div class="box-content">
       ...
       <?php echo form_input('TGL_BUKA', !empty($default->TGL_BUKA) ? $default->TGL_BUKA : '', 'class="span3 input-append date form_datetime"'); ?>           
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <div class="row-fluid">
                       <?php if ($this->session->userdata('roles_id') == 20 || $this->session->userdata('roles_id') == 34) {
                            echo form_dropdown('BLTH', $blth_options, !empty($default->BLTH) ? $default->BLTH : '');
                       } else if($this->session->userdata('roles_id') == 001 || $this->session->userdata('roles_id') == 26) {
                            if(!empty($default->BLTH)) {
                            $tahun = substr($default->BLTH,0,4);
                            $bulan = substr($default->BLTH,4);
                            }else {
                                $tahun = date('Y');
                                $bulan = date('m');
                            }
                            echo form_dropdown('BULAN', $options_bulan,$bulan,'class="span6" style="width:115px;"');
                            echo "&nbsp;";
                            echo form_dropdown('TAHUN', $options_tahun,$tahun,'class="span6" style="width:100px;"');
                       } ?>
