        ...
        <div class="row-fluid">
            <div class="span12">
                <div id ="index-content" class="well-content no-search">
                    ...
                    <div>&nbsp;</div>
                    <label><i>(*B/L = Bill of Lading)</i></label><br><br><br>
                </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body"> 
            ...
            <div class="row-fluid">
                <div class="box-content">
                    <?php
                    $form_action = base_url().'data_transaksi/perhitungan_harga_non/proses_file';
                    $hidden_form = array('id' => !empty($id) ? $id : '');
                    echo form_open_multipart($form_action, array('id' => 'fupload', 'class' => 'form-horizontal'), $hidden_form);
                    ?>
                    <div class="control-group">
                        <label for="control-label" class="control-label">Tanggal B/L : </label>
                        <div class="controls">
                        ...
                        <?php  
                            if ($this->laccess->is_prod()){ ?>
                                <div class="controls" id="dokumen">
                                    <a href="javascript:void(0);" id="lihatdoc" onclick="lihat_dokumen(this.id)" data-modul="KONTRAKPEMASOK" data-url="<?php echo $url_getfile;?>" data-filename="" id="LINK_FILE"><b>Lihat Dokumen</b></a>
                                </div> 
                        <?php } else { ?>
                                <div class="controls" id="dokumen">
                                    <a href="<?php echo base_url().'assets/upload/kontrak_pemasok/'.$default->PATH_FILE;?>" target="_blank" id="LINK_FILE"><b>Lihat Dokumen</b></a>
                                </div>
                        <?php } ?>
                        <!-- end dokumen -->
                    </div>                                      
...
<script type="text/javascript">
    ...
    $.extend($.fn.select2.defaults, {
        formatSelectionTooBig: function (limit) {
            return 'Max 20 data yang dipilih';
        }
    });
    ...
    function get_detail_file(vId) {
        ...
        $.post("<?php echo base_url()?>data_transaksi/perhitungan_harga_non/get_detail_file/", data, function (data) {
            ...
            if (rest.length > 0){
                ...
                <?php  
                    if ($this->laccess->is_prod()){ ?>
                        $("#LINK_FILE").attr("data-filename", _nama_file);
                <?php } else { ?>
                        $("#LINK_FILE").attr("href", url);
                <?php } ?>
                // <!-- end dokumen -->
    ...
    function simpan_file() {
      // var url = "<?php echo base_url() ?>data_transaksi/perhitungan_harga_non/proses_file";
