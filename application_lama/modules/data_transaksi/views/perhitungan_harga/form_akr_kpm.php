    ...
    <div class="box-content">
        ...
        echo form_open_multipart($form_action, array('id' => 'finput', 'class' => 'form-horizontal'), $hidden_form);
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <?php echo form_input('periode', !empty($default->PERIODE) ? $default->PERIODE : '', 'class="span4 cek_edit_cmb" placeholder="Pilih Periode Perhitungan" id="periode"'); ?>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <?php echo form_input('tglawal', !empty($default->TGLAWAL) ? $default->TGLAWAL : '', 'class="form_datetime cek_edit_cmb" placeholder="Dari Tanggal" id="tglawal" style="width: 120px" '); ?>
                            <label for="">s/d</label>
                    <?php echo form_input('tglakhir', !empty($default->TGLAKHIR) ? $default->TGLAKHIR : '', 'class="form_datetime cek_edit_cmb" placeholder="Sampai Tanggal" id="tglakhir" style="width: 120px" '); ?>
                </div>
            </div>
            ...
            <div class="control-group" id="divHsdDefault" hidden>
                <label class="control-label">HSD (default) : </label>
                <div class="controls">
                    <span style="display:inline-block">
                        <label for="ak_alpha" style="display:block">Alpha</label>
                        <input type="text" name="" class="form-control cls_alfa" style="width: 75px" placeholder="Alpha" id="ak_alpha" value="<?php echo !empty($default->ALPHAHSD) ? $default->ALPHAHSD : ''?>"> <bil id="bil" hidden>%</bil>
                    </span>
                    <span style="display:inline-block">
                    ...
                    <span style="display:inline-block">
                        ...
                        <input type="text" name="" class="form-control des4" placeholder="Konversi" style="width: 80px" id="ak_konversi" value="<?php echo !empty($default->KONVERSI_HSD) ? $default->KONVERSI_HSD : ''?>">
                    </span>
                    <span style="display:inline-block">
...
<script type="text/javascript">
    ...
    $('#ak_alpha').inputmask("numeric", {radixPoint: ",",groupSeparator: ".",digits: 5,autoGroup: true,prefix: '',rightAlign: false,allowMinus: false, oncleared: function () { self.Value(''); }
    });    
    ...
    $("#addButton").click(function () {
        ...
        var text_alpha = '<span style="display:inline-block">'+
                         ...
                         '<input type="text" name="ak_alpha_ke'+counter+'" class="form-control rp cek_edit" style="width: 75px" placeholder="Alpha" id="ak_alpha_ke'+counter+'" value="<?php echo !empty($default->ALPHAHSD) ? $default->ALPHAHSD : ''?>">'+ '<bil id="bil_ke'+counter+'" hidden>%</bil>'+
                         '</span>';

        var text_persen = '<span style="display:inline-block">'+
                          '<label for="bilangan_ke'+counter+'" style="display:block"></label>'+
                          '<select class="form-control cek_edit_cmb" style="width: 50px" id="bilangan_ke'+counter+'" name="bilangan_ke'+counter+'" style="display: none">'+
        ...
        var text_sulfur = '<span style="display:inline-block">'+
                          ...
                          '<input type="text" name="ak_sulfur_ke'+counter+'" class="form-control rp cek_edit" placeholder="Sulfur" style="width: 75px" id="ak_sulfur_ke'+counter+'" value="<?php echo !empty($default->SULFURHSD) ? $default->SULFURHSD : ''?>">%'+
                          '</span>';

        var text_konversi = '<span style="display:inline-block">'+
                            '<label for="ak_konversi_ke'+counter+'" style="display:block">Konversi (L)</label>'+
                            '<input type="text" name="ak_konversi_ke'+counter+'" class="form-control rp cek_edit" placeholder="Konversi" style="width: 80px" id="ak_konversi_ke'+counter+'" value="<?php echo !empty($default->KONVERSI_HSD) ? $default->KONVERSI_HSD : ''?>">'+
                            '</span>';
       
        var text_oat = '<span style="display:inline-block">'+
                       '<label for="ak_oa_ke'+counter+'" style="display:block">OAT</label>'+
                       '<input type="text" name="ak_oa_ke'+counter+'" class="form-control rp cek_edit" placeholder="OAT" style="width: 70px" id="ak_oa_ke'+counter+'" value="<?php echo !empty($default->OAT) ? $default->OAT : ''?>">'+
    ...
    function get_mops_kurs(id){ 
        ...
        var urlna ="<?php echo base_url()?>data_transaksi/perhitungan_harga/get_mops_kurs_akr_kpm/";
        if(id){
            bootbox.hideAll();
    ...
    function get_mops_kurs_ulang(id){ 
        var data_kirim = 'vidtrans='+id;
        var urlna ="<?php echo base_url()?>data_transaksi/perhitungan_harga/get_mops_kurs_akr_kpm_ulang/";
        if(id){
            bootbox.hideAll();
            bootbox.modal('<div class="loading-progress"></div>');
            $.ajax({
    ...
    function get_hitung_harga() {        
      var url = "<?php echo base_url() ?>data_transaksi/perhitungan_harga/get_hitung_harga_all/";
    ...
    function get_hitung_harga_ulang(){ 
        ...
        //     if(e){
                ...
                var url = "<?php echo base_url()?>data_transaksi/perhitungan_harga/get_hitung_harga_all_ulang/";
    ...
    function get_detail(vid){ 
        // bootbox.confirm('Apakah yakin akan proses perhitungan harga ?', "Tidak", "Ya", function(e) {
        //     if(e){                
                var vjenis = $('#'+vid).attr('jenis');
                var vid_pilih = $('#'+vid).attr('id_pilih');
                var vnopjbbm = $('#'+vid).attr('nopjbbbm');
                var vid_pemasok = $('#'+vid).attr('id_pemasok');
                var vkd_pemasok = $('#'+vid).attr('kd_pemasok');

                var data_kirim = {vid: vid_pilih, vjenis:vjenis};

                var url = "<?php echo base_url()?>data_transaksi/perhitungan_harga/get_detail/";
                ...
                dataType:'json',
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function () {
                            $("#NOPJBBM_KONTRAK_PEMASOK").val(vnopjbbm);
                            $('#ID_PEMASOK').val(vkd_pemasok);
                            $('#pemasok').val(vkd_pemasok);
                            ...
                            $("#ak_alpha_ke"+x).val($("#ak_alpha").val());
                            $("#ak_sulfur_ke"+x).val($("#ak_sulfur").val());
                            $("#ak_konversi_ke"+x).val($("#ak_konversi").val());
                            $("#ak_oa_ke"+x).val($("#ak_oa").val());
