<div class="row-fluid">
    ...
    <div class="box-content">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <?php echo form_dropdown('BLTH', $options_blth, !empty($default->BLTH) ? $default->BLTH : '', 'class="span6"'); ?>
            </div>
        </div>  
...
<script type="text/javascript">
    jQuery(function($) {
        ...
        function setDefaultBlth(){
            $('select[name="BLTH"]').empty();
            $('select[name="BLTH"]').append('<option value="">--Pilih BLTH--</option>');
        }

        $('select[name="ID_PEMASOK"]').on('change', function() {
            var stateID = $(this).val();
            var vlink_url = '<?php echo base_url()?>data_transaksi/rollback_harga/get_blth/'+stateID;
            setDefaultBlth();
            if(stateID) {
                $.ajax({
                    url: vlink_url,
                    ...
                    success:function(data) {
                        $.each(data, function(key, value) {
                            $('select[name="BLTH"]').append('<option value="'+ value.IDTRANS +'">'+ value.PERIODE +'</option>');
                        });
                    }
                });
            }
