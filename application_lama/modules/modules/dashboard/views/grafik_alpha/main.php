                    ...
                    <table id="table" style="width:100%">
                        <thead>
                            <tr>
                                <th>ALPHA BBM</th>
                                <?php                                                   
                                $arr = array('Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember', 'Rata-Rata');
                                foreach ($arr as $key) {
                                   echo "<th>".$key."</th>";
                                }
                                ?>  
...
<script type="text/javascript">
    ...
    function getDataAlpha(thn,bln) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('dashboard/grafik_alpha/getDataAlpha'); ?>",
            data: {
                "TAHUN": thn,
                "BULAN": bln,
            },
    ...
    function setGrafik(response) {
        ...
        if (obj == "" || obj == null) {
            ...
            $.each(obj, function(index, val){
                ...
                // var avg = [val.BLN01,val.BLN02,val.BLN03,val.BLN04,val.BLN05,val.BLN06,val.BLN07,val.BLN08,val.BLN09,val.BLN10,val.BLN11,val.BLN12]

                // var total = 0;

                // for(var i = 0; i<avg.length; i++){
                //     total += parseFloat(avg[i])
                //     var hasil = total / avg.length;
                // }
                
                series_data.name = val.PERIODE;
                series_data.data = bulanVar(val,maxBln);
                // series_data.rata2 = hasil;
                // series_data.rata2Hsd = val.ALPHA_HSD_AVG;
                // series_data.rata2Mfo = val.ALPHA_MFO_AVG;
                // series_data.rata2MfoLsfo = val.ALPHA_MFO_LSFO_AVG;
    ...
    function setTable(response) {
        ...
        var objList = [];
        var a = 1;
        dataSet.slice(0,3).forEach(function(item) {
            var objs = [];
    ...
    function getBulan() {
        var vlink_url = '<?php echo base_url()?>dashboard/grafik_alpha/getBulan/';
        $.ajax({
            url: vlink_url,
            type: "GET",
            dataType: "json",
