...
class perhitungan_harga_model extends CI_Model {
    ...
    public function data($key = '') {
        ...
        $this->db->join('DATA_SETTING s', "s.VALUE_SETTING = a.STATUS_APPROVE AND s.KEY_SETTING='STATUS_APPROVE_HARGA' ",'left');
        ...
        if ($_POST['BULAN'] !='') {
            $this->db->where("SUBSTRING(PERIODE, 5, 2)=",$_POST['BULAN']);
        }
        if ($_POST['TAHUN'] !='') {
            $this->db->where("SUBSTRING(PERIODE, 1, 4)=",$_POST['TAHUN']);
        }
    ...
    public function data_table($module = '', $limit = 20, $offset = 1) {
        ...
        foreach ($record->result() as $row) {
            ...
            if ($row->KODE_PEMASOK==001){ //pertamina
                $val = $row->PERIODE.'~'.$row->PEMASOK.'~'.$row->NOPJBBM.'~FOB';
                if (($row->STATUS_APPROVE>=8) && ($row->STATUS_APPROVE<=12)){
                    ...
                    $aksi = anchor(null, '<i class="icon-zoom-in" title="Lihat Data"></i>', array('class' => 'btn transparant', 'id' => 'button-cetak-' . $id, 'onclick' => 'load_form(this.id)', 'data-source' => base_url($module . '/view_pertamina/' . $id)));
                }

                // if ($row->STATUS_APPROVE==2){
                    $aksi.= anchor(null, '<i class="icon-print" title="Cetak Form Harga BBM"></i>', array('class' => 'btn transparant', 'id' => 'button-cetakp-' . $id, 'onclick' => 'load_pdf(this.id)', 'data-source' => base_url($module . '/export_pdf_pertamina')));                
                // }
                if ($row->STATUS_APPROVE==2){
                    $aksi.= anchor(null, '<i class="icon-upload-alt" title="Upload Form Harga BBM"></i>', array('class' => 'btn transparant', 'id' => 'button-uploadprt-' . $id, 'val'=> $val, 'nama-file'=> $row->PATH_FILE_UPLOAD, 'onclick' => 'load_upload(this.id)', 'data-source' => base_url($module . '/export_pdf_pertamina'), 'id-group' => $row->IDGROUP));
                }                    
            } else {
                $val = $row->PERIODE.'~'.$row->PEMASOK.'~'.$row->NOPJBBM.'~CIF';
                if (($row->STATUS_APPROVE>=8) && ($row->STATUS_APPROVE<=12)){
    ...
    public function data_table_pencarian($module = '', $limit = 20, $offset = 1) {
        $limit_a = ($offset * $limit) - $limit;
        $page = " LIMIT ".$limit_a." , ".$limit;

        $q = "
        select
            ab.*, p.NAMA_PEMASOK, p.KODE_PEMASOK
        from
            (
            select
                a.ID_KONTRAK_PEMASOK,
                '' ID_ADENDUM_PEMASOK ,
                a.ID_PEMASOK,
                a.NOPJBBM_KONTRAK_PEMASOK,
                a.JUDUL_KONTRAK_PEMASOK,
                a.PERIODE_AWAL_KONTRAK_PEMASOK,
                a.PERIODE_AKHIR_KONTRAK_PEMASOK
            from
                DATA_KONTRAK_PEMASOK a
            where
                a.PERIODE_AKHIR_KONTRAK_PEMASOK >= now()
                and a.ID_KONTRAK_PEMASOK not in (
                select
                    a2.ID_KONTRAK_PEMASOK
                from
                    ADENDUM_KONTRAK_PEMASOK a2 )
        union all
            select
                *
            from
                (
                select
                    b.ID_KONTRAK_PEMASOK,
                    b.ID_ADENDUM_PEMASOK ,
                    b2.ID_PEMASOK,
                    b2.NOPJBBM_KONTRAK_PEMASOK,
                    b.JUDUL_ADENDUM_PEMASOK,
                    b.PERIODE_AWAL_ADENDUM_PEMASOK,
                    b.PERIODE_AKHIR_ADENMDUM_PEMASOK
                from
                    ADENDUM_KONTRAK_PEMASOK b
                join DATA_KONTRAK_PEMASOK b2 on
                    b2.ID_KONTRAK_PEMASOK = b.ID_KONTRAK_PEMASOK
                where
                    b.PERIODE_AKHIR_ADENMDUM_PEMASOK >= now()
                order by
                    ID_ADENDUM_PEMASOK desc ) sb
            group by
                sb.ID_KONTRAK_PEMASOK ) ab  join MASTER_PEMASOK p on p.ID_PEMASOK = ab.ID_PEMASOK ";

        $where = "";

        $kata_kunci = $this->laccess->ai($this->input->post('kata_kunci'));
        if ($kata_kunci){
            $where = " WHERE ab.NOPJBBM_KONTRAK_PEMASOK LIKE '%{$kata_kunci}%' OR p.NAMA_PEMASOK LIKE '%{$kata_kunci}%' OR ab.JUDUL_KONTRAK_PEMASOK LIKE '%{$kata_kunci}%' " ;    
        }

        $q.=$where;
        $order = " ORDER BY ab.PERIODE_AWAL_KONTRAK_PEMASOK DESC ";

        $q_count = "select count(*) total from (".$q.") c";
        
        $total_ = $this->db->query($q_count)->result_array();
        $total = $total_[0]['total'];        

        $record = $this->db->query($q.$order.$page);
        ...
        foreach ($record->result() as $row) {
            ...
            if ($row->ID_ADENDUM_PEMASOK){
                $id_pilih = $row->ID_ADENDUM_PEMASOK;
                $aksi = anchor(null, '<i class="icon-check" title="Pilih Kontrak"></i>', array('class' => 'btn transparant', 'id' => ''. $id, 'onclick' => 'pilih_kontrak(this.id)', 'data-source' => base_url($module . '/edit/' . $id), 'jenis'=>'adendum', 'id_pilih'=>$id_pilih, 'id_pemasok'=>$row->ID_PEMASOK, 'kd_pemasok'=>$row->KODE_PEMASOK, 'nopjbbbm'=>$row->NOPJBBM_KONTRAK_PEMASOK));                
            } else {
                $id_pilih = $row->ID_KONTRAK_PEMASOK;
                $aksi = anchor(null, '<i class="icon-check" title="Pilih Kontrak"></i>', array('class' => 'btn transparant', 'id' => ''. $id, 'onclick' => 'pilih_kontrak(this.id)', 'data-source' => base_url($module . '/edit/' . $id), 'jenis'=>'awal', 'id_pilih'=>$id_pilih, 'id_pemasok'=>$row->ID_PEMASOK, 'kd_pemasok'=>$row->KODE_PEMASOK, 'nopjbbbm'=>$row->NOPJBBM_KONTRAK_PEMASOK));    
            }
    ...
    public function data_table_pencarian_lama($module = '', $limit = 20, $offset = 1) {
        $filter = array();
        // $kata_kunci = $this->laccess->ai($this->input->post('kata_kunci'));
    ...
    public function options_status() {
        ...
        $this->db->where('KEY_SETTING','STATUS_APPROVE_HARGA');
    ...
    public function get_detail(){ 
        ...
        if ($jenis=='awal'){
            $q = "SELECT 
                a.ID_KONTRAK_PEMASOK, a.JUDUL_KONTRAK_PEMASOK,
                ...
                AND a.ID_KONTRAK_PEMASOK='$id' ORDER BY d.ID_PP ";                
        } else {
            $q = "SELECT 
                a.ID_KONTRAK_PEMASOK, a.JUDUL_ADENDUM_PEMASOK JUDUL_KONTRAK_PEMASOK,                
                d.SLOC, m4.LEVEL4, m3.STORE_SLOC, m3.LEVEL3, m2.PLANT, m2.LEVEL2, m1.COCODE, m1.LEVEL1, r.ID_REGIONAL, r.NAMA_REGIONAL
                FROM ADENDUM_KONTRAK_PEMASOK a                
                LEFT JOIN ADENDUM_KONTRAK_PEMASOK_PEMBANGKIT d ON d.ID_ADENDUM_PEMASOK = a.ID_ADENDUM_PEMASOK
                LEFT JOIN MASTER_LEVEL4 m4 ON m4.SLOC = d.SLOC
                LEFT JOIN MASTER_LEVEL3 m3 ON m3.STORE_SLOC = m4.STORE_SLOC
                LEFT JOIN MASTER_LEVEL2 m2 ON m2.PLANT = m3.PLANT
                LEFT JOIN MASTER_LEVEL1 m1 ON m1.COCODE = m2.COCODE
                LEFT JOIN MASTER_REGIONAL r ON r.ID_REGIONAL = m1.ID_REGIONAL
                WHERE  a.ID_ADENDUM_PEMASOK='$id' ORDER BY d.ID_PP ";              
        }
        // print_r($q); die;
    ...
    public function get_detail_edit(){ 
        ...
        h.JNS_KURS, h.NOPJBBM, h.KODE_PEMASOK
        ...
        LEFT JOIN MASTER_LEVEL1 m1 ON m1.COCODE = m2.COCODE
        LEFT JOIN MASTER_REGIONAL r ON r.ID_REGIONAL = m1.ID_REGIONAL
        WHERE h.IDGROUP='$id' ORDER BY h.TGLINSERT ";
    ...
    public function get_detail_edit_lama(){ 
        $id = $_POST['vid'];
    ...
    public function cek_akr_kpm($id){ 
        $q = "SELECT * FROM TRANS_HITUNG_HARGA WHERE IDTRANS='$id' AND NOPJBBM <>'' ";
    ...
    public function get_akr_kpm($IDGROUP){ 
        $q = "SELECT * FROM TRANS_HITUNG_HARGA WHERE IDGROUP='$IDGROUP' ORDER BY TGLINSERT ASC ";
    ...
    //     $query = "call temp_average('$createby', '$tglawal', '$tglakhir', '$alpha', '$sulfur_hsd', '$sulfur_mfo', '$konversi_hsd', '$konversi_mfo', '$pemasok', '$oat', '$sloc', '$no_pjbbm', '$periode','$vket','$avg_mops','$jns_kurs')";
    ...
    public function call_hitung_harga($createby, $tglawal, $tglakhir, $alpha_hsd=0, $alpha_mfo=0, $alpha_mfo_lsfo=0, $sulfur_hsd=0 ,$sulfur_mfo=0, $sulfur_mfo_lsfo=0,$konversi_hsd=0, $konversi_mfo=0, $konversi_mfo_lsfo=0, $ppn, $pemasok, $oat=0, $sloc='', $no_pjbbm='', $periode='',$vket='',$avg_mops=0,$jns_kurs=0){

        $query = "call temp_average('$createby', '$tglawal', '$tglakhir', '$alpha_hsd', '$alpha_mfo', '$alpha_mfo_lsfo', '$sulfur_hsd', '$sulfur_mfo', '$sulfur_mfo_lsfo', '$konversi_hsd', '$konversi_mfo', '$konversi_mfo_lsfo', '$ppn', '$pemasok', '$oat', '$sloc', '$no_pjbbm', '$periode','$vket','$avg_mops','$jns_kurs')";
    ...
    public function call_hitung_harga_pertamina_ulang($createby, $tglawal, $tglakhir, $alpha_hsd=0, $alpha_mfo=0, $alpha_mfo_lsfo=0, $sulfur_hsd=0 ,$sulfur_mfo=0, $sulfur_mfo_lsfo=0,$konversi_hsd=0, $konversi_mfo=0, $konversi_mfo_lsfo=0, $ppn, $pemasok, $oat=0, $sloc='', $no_pjbbm='', $vidtrans='', $periode='',$vket='',$avg_mops=0,$jns_kurs=0){

        $query = "call edit_average('$createby', '$tglawal', '$tglakhir', '$alpha_hsd', '$alpha_mfo', '$alpha_mfo_lsfo', '$sulfur_hsd', '$sulfur_mfo', '$sulfur_mfo_lsfo', '$konversi_hsd', '$konversi_mfo', '$konversi_mfo_lsfo', '$ppn', '$pemasok', '$oat', '$sloc', '$no_pjbbm', '$vidtrans', '$periode','$vket','$avg_mops','$jns_kurs')";
    ...
    public function call_hitung_harga_arr($createby, $tglawal, $tglakhir, $alpha=0, $sulfur_hsd=0 ,$sulfur_mfo=0,$konversi_hsd=0, $konversi_mfo=0, $pemasok, $oat=0, $sloc='', $no_pjbbm='', $periode='',$vket='',$avg_mops=0,$jns_kurs=0){
    ...
    public function call_hitung_harga_arr_ulang($createby, $tglawal, $tglakhir, $alpha=0, $sulfur_hsd=0 ,$sulfur_mfo=0,$konversi_hsd=0, $konversi_mfo=0, $pemasok, $oat=0, $sloc='', $no_pjbbm='', $vidtrans='', $periode='',$vket='',$avg_mops=0,$jns_kurs=0){

        $query = "call edit_average('$createby', '$tglawal', '$tglakhir', '$alpha', '$sulfur_hsd', '$sulfur_mfo', '$konversi_hsd', '$konversi_mfo', '$pemasok', '$oat', '$sloc', '$no_pjbbm', '$vidtrans','$periode','$vket','$avg_mops','$jns_kurs')";
    ...
    public function call_mops_kurs($vidtrans=''){
        $query = "call temp_mops_kurs('$vidtrans')";
    ...
    public function call_simpan_data($vidtrans='',$vidgroup='',$vidkoreksi='',$vstatus='',$path_file=''){
        $query = "call save_mops_hitung('$vidtrans','$vidgroup','$vidkoreksi','$vstatus','$path_file')";
    ...
    public function call_simpan_data_edit($vidtrans='',$vidtrans_edit='',$vidgroup='',$idkoreksi='',$path_file='' ){
        $query = "call save_edit_mops_hitung('$vidtrans','$vidtrans_edit','$vidgroup','$idkoreksi','$path_file')";
    ...
    public function call_kirim_data($p_trans='', $p_status='', $p_by_user='', $p_ket='', $p_idkoreksi=''){
        $query = "CALL PROSES_HITUNG_HARGA('$p_trans', '$p_status', '$p_by_user','$p_ket','$p_idkoreksi'); ";
    ...
    public function get_data_setting(){ 
        $q = "SELECT * FROM DATA_SETTING WHERE KEY_SETTING='SULFUR_HSD' ORDER BY NAME_SETTING ASC";
    ...
    public function cek_periode_pertamina($PERIODE='',$vidtrans=''){ 
        ...
        $q = "SELECT p.NAMA_PEMASOK FROM TRANS_HITUNG_HARGA h
    ...
    public function get_id_trans($IDGROUP){ 
        $q = "SELECT IDTRANS FROM TRANS_HITUNG_HARGA WHERE IDGROUP='$IDGROUP' ORDER BY TGLINSERT ASC";
    ...
    public function set_notif_tolak($username,$status='0'){
        if ($status){
            $cek_tatus = " AND STATUS_APPROVE='$status' ";
        } else {
            $cek_tatus = "";
        }
        
        $q = "UPDATE TRANS_HITUNG_HARGA SET IS_TOLAK='0' WHERE  CREATE_BY='$username' ".$cek_tatus;
    ...
    public function get_last_update($id){ 
        $q = "SELECT * FROM HISTO_TR_HARGA WHERE IDTRANS='$id' ORDER BY UPDATE_DATE DESC LIMIT 1";
        $query = $this->db->query($q)->result_array();
        $this->db->close();
        return $query;
    }
