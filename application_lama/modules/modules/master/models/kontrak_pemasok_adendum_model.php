...
class kontrak_pemasok_adendum_model extends CI_Model {
    ...
    public function data_awal($key = '') {
        ...
        CD_KONTRAK_PEMASOK, CD_BY_KONTRAK_PEMASOK, UD_KONTRAK_PEMASOK, ISAKTIF_KONTRAK_PEMASOK, SLOC, SKEMA_ISIDENTIL";
    ...
    public function save_as_new($data,$nama_file) {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            //jika jenis CIF
            if ($data['JENIS_AKHIR_ADENDUM_PEMASOK']==1){
                ...
                foreach($var as &$row) {
                    $arr['ID_ADENDUM_PEMASOK'] = $id_kontrak;
                    $arr['SLOC'] = $row;

                    $arr_SLOC[] = $arr;
                }

                $this->db->delete('ADENDUM_KONTRAK_PEMASOK_PEMBANGKIT', array('ID_ADENDUM_PEMASOK' => $id_kontrak));
                $this->db->insert_batch('ADENDUM_KONTRAK_PEMASOK_PEMBANGKIT', $arr_SLOC);                
            }
    ...
    public function save($data, $key) {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            if ($data['JENIS_KONTRAK_PEMASOK']==1){
                ...
                foreach($var as &$row) {
                    $arr['ID_ADENDUM_PEMASOK'] = $key;
                    $arr['SLOC'] = $row;

                    $arr_SLOC[] = $arr;
                }

                $this->db->delete('ADENDUM_KONTRAK_PEMASOK_PEMBANGKIT', array('ID_ADENDUM_PEMASOK' => $key));
                $this->db->insert_batch('ADENDUM_KONTRAK_PEMASOK_PEMBANGKIT', $arr_SLOC);                
            }
    ...
    public function data_table($module = '', $limit = 20, $offset = 1) {
        ...
        foreach ($record->result() as $row) {
            ...
            $aksi = anchor(null, '<i class="icon-zoom-in" title="Lihat Kontrak"></i>', array('class' => 'btn transparant', 'id' => 'button-view3-' . $id, 'onclick' => 'load_form(this.id)', 'data-source' => base_url($module . '/edit_adendum_view/' . $id)));
            $x=$no;
