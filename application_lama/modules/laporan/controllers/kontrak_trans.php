 ...
 */
class kontrak_trans extends MX_Controller {

    private $_title = 'KONTRAK TRANSPORTIR UNIT';
    private $_limit = 10;
    private $_module = 'laporan/kontrak_trans';
    ...
    public function __construct() {
        ...
        /* Load Global Model */
        $this->load->model('kontrak_trans_model', 'tbl_get');
    }
    ...
    public function load($page = 1) {
        ...
        $table->align = array('NO' => 'center', 'NAMA_REGIONAL' => 'left', 'LEVEL1' => 'left', 'LEVEL2' => 'left', 'LEVEL3' => 'left', 'LEVEL4' => 'left', 'NAMA_TRANSPORTIR' => 'left', 'KD_KONTRAK_TRANS' => 'center', 'TGL_KONTRAK_TRANS' => 'center', 'TGL_KONTRAK_TRANS_AKHIR' => 'center');
        $table->page = $page;
    ...
    public function ajax_list(){
        $data['p_unit'] = $this->input->post('p_unit');
        $data['p_transportir'] = $this->input->post('p_transportir');
        $data['p_tglawal'] = $this->input->post('p_tglawal');
        $data['p_tglakhir'] = $this->input->post('p_tglakhir');
        $data['p_cari'] = $this->input->post('p_cari');
    ...
    public function get_list() {
        ...
        $list = $this->tbl_get->get_list_adendum($id,$value);
        $data = array();
        $no = $_POST['start'];
        foreach ($list->result() as $table) {
            ...
            $row[] = $table->DEPO;
            $row[] = $table->TRANSIT;
            $row[] = $table->PEMBANGKIT;
            $row[] = $table->JALUR;
            $row[] = number_format($table->JARAK,2,',','.');
            $row[] = number_format($table->HARGA,2,',','.');
        ...
        $output = array(
                        ...
                        "recordsFiltered" => $this->tbl_get->count_all_adendum(),  //$this->tbl_get->count_filtered(),
                        "data" => $data,
                );
    ...
    public function ajax_list_adendum(){
        ...
        $list = $this->tbl_get->get_datatables_adendum($p_kode,$p_adendum);        
        echo json_encode($list);
    }    
    ...
    public function export_excel_adendum(){
        extract($_POST);
        $data['p_kode'] = $xNO_KONTRAK; 
        $data['p_adendum'] = $xID;
        $data['data'] = $this->tbl_get->get_datatables_adendum_export($data);
        $data['JENIS'] = 'XLS';
