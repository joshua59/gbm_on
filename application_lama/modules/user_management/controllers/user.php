...
class user extends MX_Controller {
    ...
    public function index() {
        ...
        $data['role_options'] = $this->user_model->options_role();
    ...
    public function load($page = 1) {
        ...
        $table->align = array('no' => 'center', 'aksi' => 'center', 'user_status' => 'center', 'is_ldap' => 'center', 'status_password' => 'center');
        $table->page = $page;
    ...
    public function username_check_ldap($str=''){
        $this->form_validation->set_message('username_check_ldap', 'Status User LDAP dikhususkan untuk user dengan email korporat @pln.co.id');
        return FALSE;
    }

    public function username_check($str=''){
        $this->form_validation->set_message('username_check', 'Status User Non LDAP dikhususkan untuk user selain email korporat @pln.co.id');
        return FALSE;
    }
    ...
    public function proses() {
		...
		if ($this->laccess->otoritas('add') || $this->laccess->otoritas('edit')) {
            ...
            $email_cek = $this->input->post("email_user");
            if ($email_cek){
                $pos = strpos($email_cek, '@pln.co.id');
                if ($pos === false) {
                    if ($this->input->post("IS_LDAP")){
                        $this->form_validation->set_rules('email_user', 'email_user', 'callback_username_check_ldap');
                    }
                } else {
                    if (!$this->input->post("IS_LDAP")){
                        $this->form_validation->set_rules('email_user', 'email_user', 'callback_username_check');
                    }
                }
            }
			...
			if ($this->form_validation->run($this)) {
                ...
                // $IS_LDAP = $this->input->post("IS_LDAP");
                if ($this->input->post("IS_LDAP")){
                    $IS_LDAP = '1';
                } else {
                    $IS_LDAP = '0';
				...
				$data = $this->user_model->save_as_new($level[0], $kduser, $nama, $username, $pwd, $email, $level[1], $kodelevel, $isaktif, $IS_LDAP, $id)->row();
				$rc = $data->RCDB;
				if($rc == "RC00"){
    ...
    public function ganti_password() {
	 ...
	 public function profil() {
        ...
        $unit = $this->user_model->get_unit();
        if($data['default']->LEVEL_USER == 'R') {
            $data['nama_unit'] = $unit[0]['NAMA_REGIONAL'];
            $data['level'] = 'Level Regional';
        } else if($data['default']->LEVEL_USER == 1) {
            $data['nama_unit'] = $unit[0]['LEVEL1'];
            $data['level'] = 'Level 1';
        } else if($data['default']->LEVEL_USER == 2) {
            $data['nama_unit'] =  $unit[0]['LEVEL2'];
            $data['level'] = 'Level 2';
        } else if($data['default']->LEVEL_USER == 3) {
            $data['nama_unit'] =  $unit[0]['LEVEL3'];
            $data['level'] = 'Level 3';
        } else if($data['default']->LEVEL_USER == 4) {
            $data['nama_unit'] =  $unit[0]['LEVEL4'];
            $data['level'] = 'Level 4';
        } else if($data['default']->LEVEL_USER == 0){
            $data['nama_unit'] = 'PUSAT';
            $data['level'] = 'Pusat';
        }
	...
	public function proses_profil() {
        $this->form_validation->set_rules('kode_user', 'NIP', 'trim|required|min_length[5]|max_length[30]');
